...
class grid_search_cross_validation(object):
    ...
    def _cross_validation(self,xTr, yTr, indices):
        ...
        training_losses = []
        validation_losses = []
        num_parameters = np.prod([len(x) for x in self.parameter_dict.values()])
        ...
        for i,index in enumerate(indices):
            ...
            xVal_i = xTr[(index[1])]
            yVal_i = yTr[(index[1])]
            best_model, train_losses, val_losses = self._grid_search(xTr_i,yTr_i,xVal_i,yVal_i)
            k_fold_train_losses[:,i] = train_losses 
            k_fold_val_losses[:,i] = val_losses
            self.best_models.append(best_model)

        self.mean_training_losses = np.mean(k_fold_train_losses,axis=0)
        self.mean_validation_losses = np.mean(k_fold_val_losses,axis=0)
        parameters_min_val_loss = np.where(self.mean_validation_losses == np.array(self.mean_validation_losses).min())[0][0]
        self.optimal_model = self.best_models[parameters_min_val_loss]
    ...
    def _grid_search(self,xTr, yTr, xVal, yVal):
        ...
        parameter_grid = param_grid(self.parameter_dict)
